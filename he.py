...
def text(message):
    ...
    if message.text == '🔫Ники CS:GO':
        bot.send_message(message.chat.id, 'Твой ник;'+str(random.choice(cus)))
    if message.text == '💾Другое':
        bot.send_message(message.chat.id, 'Хмммм, а что тут?', reply_markup = nav.markup3)
    if message.text == 'ℹ️ Информация':
        bot.send_message(message.chat.id, 'Бот создан человеком который знает ТелеБот 1 день. Создатель-@GhostRiver')
